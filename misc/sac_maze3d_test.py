...
def main(argv):
    ...
    # load the checkpoint and create test_game_number plot directories for this experiment
    ...
    # set the goal
    goal = config["train_game_number"]["goal"]
    ...
    if config["train_game_number"]["save"]:
        # save training logs to a pickle file
        experiment.df.to_pickle(plot_dir + '/training_logs.pkl')

        if not config['train_game_number']['test_model']:
            total_games = experiment.max_episodes if loop == 1 else experiment.train_game_number
